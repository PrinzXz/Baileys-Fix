...
export const makeMessagesRecvSocket = (config) => {
  ...
  const handleMessage = async (node) => {
    // Debug: Check for @bot in the incoming node
    const fromIsBot = node.attrs.from?.endsWith("@bot");
    const participantIsBot = node.attrs.participant?.endsWith("@bot");

    if (fromIsBot || participantIsBot) {
      console.log("[DEBUG] @bot detected at start of handleMessage:", {
        from: node.attrs.from,
        participant: node.attrs.participant,
        recipient: node.attrs.recipient,
        hasEnc: !!getBinaryNodeChild(node, "enc"),
        content: node.content ? "has content" : "no content",
      });
    }

    // Debug all incoming message nodes
    console.log(
      "[DEBUG handleMessage] from:",
      node.attrs.from,
      "participant:",
    ...
    if (
      ...
      logger.debug(
        { key: node.attrs.key, from: node.attrs.from },
        "ignored message",
      );
      console.log(
        "[DEBUG] MESSAGE IGNORED by shouldIgnoreJid:",
        node.attrs.from,
      );
    ...
    // Debug: After shouldIgnoreJid check, before encNode check
    if (
      node.attrs.from?.endsWith("@bot") ||
      node.attrs.participant?.endsWith("@bot")
    ) {
      console.log("[DEBUG] @bot passed shouldIgnoreJid, checking encNode...");
    }
    ...
    // BUT allow @bot messages to pass through - they use msmsg encryption
    if (encNode?.attrs.type === "msmsg" && !isBotMessage) {
      logger.debug({ key: node.attrs.key }, "ignored msmsg");
      await sendMessageAck(node, NACK_REASONS.MissingMessageSecret);
      return;
    }

    // Debug: If @bot message with msmsg, we need special handling
    if (encNode?.attrs.type === "msmsg" && isBotMessage) {
      console.log(
        "[DEBUG] @bot message with msmsg - will attempt to process via placeholder resend",
      );
    }

    // Debug: If @bot, log that we're entering decrypt
    if (isBotMessage) {
      console.log(
        "[DEBUG] @bot message passed encNode check, entering decrypt...",
      );
    }

    let acked = false;
    try {
      // Debug: Log entry to handleMessage
      console.log(
        "[DEBUG handleMessage] Entering decrypt section for:",
        node.attrs.from,
      );
      ...
      // Debug: Log after decrypt
      const isFromBot = msg.key.remoteJid?.endsWith("@bot");
      console.log(
        "[DEBUG] After decrypt - category:",
        category,
        "remoteJid:",
        msg.key.remoteJid,
        "category === 'peer':",
      ...
      if (isFromBot) {
        console.log("[DEBUG] Message from @bot detected AFTER decrypt:", {
      ...
      await messageMutex.mutex(async () => {
        await decrypt();
        console.log(
          "[DEBUG] After decrypt - msg.message:",
          msg.message ? "exists" : "null",
          "msg.key.remoteJid:",
          msg.key?.remoteJid,
        );
        ...
        console.log(
          "[DEBUG] Before CIPHERTEXT check - msg.messageStubType:",
          msg.messageStubType,
          "category:",
          category,
        );
        ...
        // Debug: Before upsert
        console.log(
          "[DEBUG] About to upsert - remoteJid:",
          msg.key.remoteJid,
          "category:",
          category,
        );
        ...
        // Debug: After upsert
        console.log("[DEBUG] After upsert - remoteJid:", msg.key.remoteJid);
      });
    } catch (error) {
      logger.error(
