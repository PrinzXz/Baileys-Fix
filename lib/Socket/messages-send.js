...
const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, quoted } = {}) => {
    ...
    const eugen = new EugenHandler(Utils, waUploadToServer, relayMessage, { logger, mediaCache: config.mediaCache, options: config.options, mediaUploadTimeoutMs: config.mediaUploadTimeoutMs, user: authState.creds.me })
    const waitForMsgMediaUpdate = bindWaitForEvent(ev, 'messages.media-update')

    return {
        ...sock,
        getPrivacyTokens, assertSessions, relayMessage, sendReceipt, sendReceipts, eugen, readMessages,
        ...
        sendPaymentMessage: (jid, data, quoted) => eugen.handlePayment({ requestPaymentMessage: data }, jid, quoted),
        sendProductMessage: (jid, data, quoted) => eugen.handleProduct({ productMessage: data }, jid, quoted),
        sendInteractiveMessage: (jid, data, quoted) => eugen.handleInteractive({ interactiveMessage: data }, jid, quoted),
        sendAlbumMessage: (jid, medias, quoted) => eugen.handleAlbum({ albumMessage: medias }, jid, quoted),
        sendEventMessage: (jid, data, quoted) => eugen.handleEvent({ eventMessage: data }, jid, quoted),
        sendPollResultMessage: (jid, data, quoted) => eugen.handlePollResult({ pollResultMessage: data }, jid, quoted),
        sendStatusMentionMessage: (jid, data, quoted) => eugen.handleStMention({ statusMentionMessage: data }, jid, quoted),
        sendOrderMessage: (jid, data, quoted) => eugen.handleOrderMessage({ orderMessage: data }, jid, quoted),
        sendGroupStatusMessage: (jid, data, quoted) => eugen.handleGroupStory({ groupStatus: data }, jid, quoted),
        sendCarouselMessage: (jid, data, quoted) => eugen.handleCarousel({ carouselMessage: data }, jid, quoted),
        sendCarouselProtoMessage: (jid, data, quoted) => eugen.handleCarouselProto({ carouselProto: data }, jid, quoted),
        stickerPackMessage: (jid, data, options) => eugen.handleStickerPack(data, jid, options?.quoted),
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = eugen.detectType(content)
            if (messageType) return await eugen.processMessage(content, jid, quoted)
