...
if (
  ...
  libsignal.SessionCipher.prototype.decryptWithSessions
) {
  libsignal.SessionCipher.prototype.decryptWithSessions = async function (
    data,
    sessions,
  ) {
    if (!sessions.length) {
      throw new libsignal.SessionError("No sessions available");
    }
    const errs = [];
    ...
    throw new libsignal.SessionError(
...
const jidToSenderKeyName = (group, user) =>
  new SenderKeyName(group, jidToAddr(user));
...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
  ...
  const migratedCache = new LRUCache({
    ttl: 7 * 24 * 60 * 60 * 1000,
    ttlAutopurge: true,
    updateAgeOnGet: true,
  });
  ...
  return {
    decryptGroupMessage: ({ group, authorJid, msg }) => {
      const cipher = new GroupCipher(
        storage,
        jidToSenderKeyName(group, authorJid),
      );
    ...
    injectE2ESession: ({ jid, session }) =>
      txn(
        () =>
          new libsignal.SessionBuilder(storage, jidToAddr(jid)).initOutgoing(
            session,
          ),
        jid,
      ),
    ...
    validateSession: async (jid) => {
      try {
        ...
        return {
          exists: sess?.haveOpenSession?.() || false,
          reason: sess ? null : "no session",
        };
      } catch (e) {
    ...
    migrateSession: async (fromJid, toJid) => {
      ...
      const uncachedDevices = userDevices.filter(
        (d) => !migratedCache.has(`${user}.${d}`),
      );
      ...
      return txn(async () => {
        const pnAddrStrs = Array.from(
          new Set(deviceJids.map((d) => jidToAddr(d.jid).toString())),
        );
        ...
        return {
          migrated,
          skipped: deviceJids.length - migrated,
...
function signalStorage({ creds, keys }, lidMapping, logger) {
  ...
  return {
    ...
    loadPreKey: async (id) => {
      ...
      return key
        ? { privKey: Buffer.from(key.private), pubKey: Buffer.from(key.public) }
        : null;
    },
    ...
    storeSenderKey: async (senderKeyName, key) => {
      ...
      const buf =
        typeof serialized === "string"
          ...
          : Buffer.from(JSON.stringify(serialized), "utf-8");
