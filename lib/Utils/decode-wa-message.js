          ...
          try {
            ...
            switch (e2eType) {
              ...
              case "plaintext":
                ...
                break;
              case "msmsg":
                // msmsg is used for Meta AI messages
                // Try to decrypt - will fail if no session exists
                try {
                  msgBuffer = await repository.decryptMessage({
                    jid: sender,
                    type: "msg",
                  ...
                  // No session record - this is expected for Meta AI bot
                  // Let it throw to be handled as CIPHERTEXT
                  throw new Error(
                    `msmsg decryption failed: ${msmsgErr.message}`,
                  );
                }
                break;
              default:
