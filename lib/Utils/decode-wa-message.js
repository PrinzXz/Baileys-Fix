...
export function decodeMessageNode(stanza, meId, meLid) {
  ...
  // Debug: log all incoming message nodes
  console.log(
    "[DEBUG decodeMessageNode] from:",
    from,
    "recipient:",
    recipient,
    "participant:",
  ...
  if (isJidMetaAI(from)) {
    // Message from Meta AI bot
    console.log(
      "[DEBUG decodeMessageNode] Detected Meta AI message from:",
      from,
    );
    msgType = "bot";
    chatId = from;
          ...
          try {
            ...
            switch (e2eType) {
              ...
              case "plaintext":
                ...
                break;
              case "msmsg":
                // msmsg is used for Meta AI messages and other special messages
                // First, check if session exists
                const botJid = sender;
                console.log(
                  "[DEBUG decode-wa-message.js] Attempting msmsg decrypt for:",
                  botJid,
                );

                // Try to decrypt using regular message decryption
                try {
                  msgBuffer = await repository.decryptMessage({
                    jid: botJid,
                    type: "msg",
                  ...
                  console.log(
                    "[DEBUG decode-wa-message.js] msmsg decryption SUCCESS",
                  );
                } catch (msmsgErr) {
                  // Check if it's a session error - try to create session first
                  if (isSessionRecordError(msmsgErr)) {
                    console.log(
                      "[DEBUG decode-wa-message.js] No session for msmsg, attempting session creation...",
                    );

                    // Try to create session by checking if we have pre-key bundle
                    // This typically requires fetching from server, which may not be available for bots
                    // For now, just let it fail and we'll handle via placeholder resend
                    console.log(
                      "[DEBUG decode-wa-message.js] Cannot create session for bot - no pre-key bundle available",
                    );
                  }

                  // For bot messages, don't throw - let it be handled as CIPHERTEXT
                  // This will trigger placeholder resend mechanism
                  console.log(
                    "[DEBUG decode-wa-message.js] msmsg decryption failed:",
                    msmsgErr.message,
                  );
                  throw new Error(
                    `msmsg decryption not supported: ${msmsgErr.message}`,
                  );
                }
                break;
              default:
