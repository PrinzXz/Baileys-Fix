...
export const prepareStickerPackMessageContent = async (stickerPack: any, options: MessageContentGenerationOptions) => {
	...
	const stickerPromises = stickers.map(async (s: any, i: number) => {
		let dataStream

		if (s.sticker?.url) {
			dataStream = await getStream({ url: s.sticker.url })
	...
	let cover
	if (typeof stickerPack.cover === 'string' && (stickerPack.cover.startsWith('http://') || stickerPack.cover.startsWith('https://'))) {
		cover = await getStream({ url: stickerPack.cover })
	...
	const { stream: coverStream } = cover
	...
	const stickerPackUpload = await encryptedStream(zipBuffer, 'sticker-pack' as any, {
	...
	try {
		...
		if ('sharp' in lib && lib.sharp) {
			...
			const jimpImage = await (lib.jimp.Jimp as any).read(coverBuffer)
...
export const generateWAMessageContent = async (
	...
	} else if (hasNonNullishProperty(message as any, 'stickerPack')) {
		m = await prepareStickerPackMessageContent((message as any).stickerPack, options)
	} else if (hasNonNullishProperty(message as any, 'tableMessage')) {
		;(m as any).richResponseMessage = {
			messageType: 1, // AI_RICH_RESPONSE_TYPE_STANDARD
			submessages: [
				{
					messageType: 4, // AI_RICH_RESPONSE_TABLE
					tableMetadata: {
						title: (message as any).tableMessage.title,
						rows: (message as any).tableMessage.rows
					}
				}
			]
		}
