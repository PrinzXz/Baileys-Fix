/**
 * @PrinzXz/Baileys-Fix - Advanced Message Handler
...
class EugenHandler {
  ...
  constructor(utils: any, waUploadToServer: any, relayMessageFn: any, options: any = {}) {
    this.utils = utils;
  ...
  detectType(content: any) {
    ...
    const matchedKey = Object.keys(types).find((k) => content[k]) as keyof typeof types;
    return matchedKey ? types[matchedKey] : null;
  }

  // ========== UNIFIED PROCESSOR ==========
  async processMessage(content: any, jid: string, quoted: any) {
  ...
  // ========== HELPERS ==========
  async prepMedia(data: any, type: string) {
    if (!data) return null;
    const payload =
      typeof data === "object" && data.url
        ? { [type]: { url: data.url } }
        : { [type]: data };
    return await this.utils.prepareWAMessageMedia(payload, {
      upload: this.upload,
    });
  }

  async genContent(jid: string, content: any, opts: any = {}) {
  ...
  async sendMsg(jid: string, message: any, opts: any = {}) {
    await this.relay(jid, message, opts);
  }

  buildCtx(quoted: any, sender: any) {
    return {
  ...
  buildFullCtx(ctx: any, adReply: any) {
    const final: any = ctx
      ? {
  ...
  async handlePayment(content: any, jid: string, quoted: any) {
    ...
    const msg = await this.genContent(
      content.jid,
      {
        requestPaymentMessage:
          this.utils.WAProto.proto.Message.RequestPaymentMessage.fromObject({
  ...
  async handleProduct(content: any, jid: string, quoted: any) {
    ...
    if (p.thumbnail) {
      const imgContent = Buffer.isBuffer(p.thumbnail)
        ? { image: p.thumbnail }
        : { image: { url: p.thumbnail.url || p.thumbnail } };
      const res = await this.utils.generateWAMessageContent(imgContent, {
        upload: this.upload,
      });
  ...
  async handleInteractive(content: any, jid: string, quoted: any) {
    // Handle both 'interactiveMessage' and 'interactive' keys
    const i = content.interactiveMessage || content.interactive || {};
    let media = null;

    if (i.thumbnail)
      media = await this.prepMedia({ url: i.thumbnail }, "image");
      ...
      if (i.jpegThumbnail)
        media.documentMessage.jpegThumbnail =
          typeof i.jpegThumbnail === "object" && i.jpegThumbnail.url
            ? { url: i.jpegThumbnail.url }
            : i.jpegThumbnail;
    ...
    if (media) {
      ...
      const headerTitle =
        typeof i.header === "string" ? i.header : i.header?.title || "";
      interactive.header = {
        title: headerTitle,
  ...
  async handleAlbum(content: any, jid: string, quoted: any) {
    ...
    const album = await this.genContent(
      jid,
      {
        messageContextInfo: { messageSecret: crypto.randomBytes(32) },
        albumMessage: {
          expectedImageCount: arr.filter((a: any) => a.image).length,
          expectedVideoCount: arr.filter((a: any) => a.video).length,
        },
      },
    ...
    for (const item of arr) {
      ...
      await this.sendMsg(jid, img.message, {
        ...
        quoted: {
          key: { ...album.key, fromMe: true, participant: this.genJid() },
    ...
    const msg = await this.genContent(
      jid,
      {
        pollResultSnapshotMessage: {
          name: p.name,
          pollVotes: (p.pollVotes || []).map((v: any) => ({
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              newsletterJid:
                p.newsletter?.newsletterJid || "120363399602691477@newsletter",
  ...
  async handleStMention(content: any, jid: string, quoted: any) {
    const d = content.statusMentionMessage;
    const media = await this.prepMedia(
      d.image || d.video,
      d.image ? "image" : "video",
    );

    const msg = await this.relay(
      "status@broadcast",
      { ...media },
      {
            ...
            content: [
              {
                ...
                attrs: {},
                content: [
                  { tag: "to", attrs: { jid: d.mentions }, content: undefined },
                ],
              },
            ],
          },
    ...
    const xontols = await this.genContent(
      jid,
      {
  ...
  async handleOrderMessage(content: any, jid: string, quoted: any) {
    ...
    if (o.thumbnail) {
      if (Buffer.isBuffer(o.thumbnail)) {
        ...
        try {
          const res = await axios.get(o.thumbnail, {
  ...
  async handleGroupStory(content: any, jid: string, quoted: any) {
    const s = content.groupStatus;
    const msgContent =
      s.message ||
      (await this.utils.generateWAMessageContent(s, { upload: this.upload }));
    const msg = {
      message: {
        groupStatusMessageV2: { message: msgContent.message || msgContent },
      },
    };
  ...
  async handleCarousel(content: any, jid: string, quoted: any) {
    const c = content.carouselMessage || content.carousel || {};
    const cards = await Promise.all(
      (c.cards || []).map((card: any) => this.buildCard(card)),
    );
      ...
      return {
        header: {
          ...
          productMessage: {
            product: {
              productImage: (
                await this.prepMedia({ url: card.imageUrl }, "image")
              ).imageMessage,
  ...
  async handleStickerPack(stickerPack: any, jid: string, quoted: any) {
